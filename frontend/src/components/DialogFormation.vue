    ...
    <v-dialog persistent v-model="dialog" width="500">
      ...
      <v-card>
        <v-card-title class="text-h5 primary white--text">
          Adicionar licença ou certificado
  ...
  methods: {
    async saveDialog() {
      ...
      if (form1 && form2) {
        ...
        const data = await this.createGraduation(obj);

        this.listFormation.push({
          ...data,
          start: {
    ...
    async createGraduation(obj) {
      ...
      const { data } = await ApiService.post("clients/api/graduation.php", tmp);
